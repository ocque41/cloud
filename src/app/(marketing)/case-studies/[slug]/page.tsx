...
import {
  caseStudyModels,
  getCaseStudyModelBySlug,
} from "@/lib/case-study-models";
import { buildMetadata } from "@/lib/metadata";

export function generateStaticParams() {
  return caseStudyModels.map((model) => ({ slug: model.slug }));
}
...
export async function generateMetadata({
  ...
  const model = getCaseStudyModelBySlug(slug);
  if (!model) {
    return buildMetadata({ title: "Model", path: `/case-studies/${slug}` });
  }

  return buildMetadata({
    title: model.name,
    description: model.short,
    path: `/case-studies/${model.slug}`,
  });
}
  ...
  return (
    <div className="container pb-24">
      <div className="space-y-6 text-center">
        ...
        <h1 className="display mx-auto max-w-4xl">{model.name}</h1>
        <p className="lead mx-auto max-w-2xl">{model.short}</p>
      </div>
      ...
      <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr]">
        <div className="space-y-10 text-left text-lg text-[color:var(--muted)]">
          ...
          <section className="space-y-3">
            ...
            <p>
              ...
              tailor this solution to your stack. Expect a detailed spec, an
        ...
        <div className="rounded-3xl border border-[color:var(--muted)]/30 bg-[#1f1f1f] p-6 shadow-[0_30px_80px_rgba(0,0,0,0.35)]">
          ...
          <div className="mt-6">
            <RequestForm defaultProduct={model.slug} />
          </div>
        </div>
      </div>
    </div>
